...
/// Runs sing-box/libbox inside the sandboxed macOS Network Extension.
final class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private func startTunnelAsync(options: [String: NSObject]?) async throws {
        ...
        let profileName =
            (providerConfig[TunnelConstants.profileNameKey] as? String) ?? "Erebrus"
        log.info(
            "startTunnel profile=\(profileName, privacy: .public) bytes=\(config.utf8.count, privacy: .public)"
        )
        ...
        let stderrPath = FilePath.cacheDirectory
        ...
        guard let server = LibboxNewCommandServer(statsMonitor, 300) else {
            throw NSError(domain: "ErebrusTunnel", code: 4, userInfo: [
                NSLocalizedDescriptionKey: "Command server creation failed",
            ])
        }
